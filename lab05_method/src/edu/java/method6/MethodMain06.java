...
public class MethodMain06 {
    ...
    //(주의) 가변 인수를 저장하는 파라미터는 반드시 가장 마지막 파라미터로 선언.(길이를 모르니까)
    //(주의) 가변 인수는 2개 이상 정의할 수 없음.
    
    //    public static int addAllIntegers(int... args, int x) (X)
    //    public static int addAllIntegers(int... x, int... args) (X)  //어디까지 x인지 알 수 없음
    //    public static int addAllIntegers(int x, int... args) (O)
    ...
    public static double calculate(String op, double... args) {
        ...
        case "+":
            ...
            break;
        case "*":
            result = 1;  //초기값 0이면 곱하기했을 때 무조건 0나옴
            for(double x : args) {
                result *= x;
            }
            break;
