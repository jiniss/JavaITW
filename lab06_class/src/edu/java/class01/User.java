...
public class User {
    ...
    //클래스를 작성할 때 개발자가 생성자를 정의하면, 자바 컴파일러는 기본 생성자를 자동으로 만들지 않음!
    //생성자의 이름은 클래스의 이름과 동일. 생성자는 리턴 타입을 사용하지 않음!
    
    //기본 생성자(default constructor) - argument를 갖지 않는 생성자
    //모든 속성들을 그 타입의 기본값으로 초기화함.
    //숫자 타입 기본값: 0,   boolean 타입 기본값: false,   참조 타입 기본값: null   (->'없다'라는 개념)
    ...
    public User(String userId, String password, int age) {  //constructor-2
        this.userId = userId;   //this = 나 자신(새로운 주소값) (내가 가지고 있는 userId에 userId값을 넣을 것)
        this.password = password;
        this.age = age;  // this. 생성된 객체를 가리키는 주소
    }
    
    public User(String userId, String password) {  //constructor-3
//        this.userId = userId;
//        this.password = password;  //age는 전달하지 않았으므로 값은 0
        this(userId, password, 0); // this() 함수처럼 사용 => 다른 생성자(constructor-2)를 호출
    }
