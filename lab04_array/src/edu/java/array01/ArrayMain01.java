...
public class ArrayMain01 {
    ...
    public static void main(String[] args) {
        ...
        // 타입[] 배열이름 = { ... };
        int[] numbers = {1, 3, 5, 7, 9};  // 아직 저장될 값이 없을 때 new 사용.
        for (int i = 0; i < numbers.length; i++) {
            System.out.println(numbers[i]);
        }
        
        System.out.println("----------");
        // 향상된 for 문(enhanced for statement). for-each 문장.(=하나씩 꺼낸다는 뜻)
        for (int n : numbers) {   // 콜론 좌측: 변수 선언/ 우측: 배열(배열이 있을 때만 사용)
